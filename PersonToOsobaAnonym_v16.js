...
try
{
	var scriptName = "PersonToOsobaAnonym";
	coort.Trace(scriptName + " START -->");
	//---------------------CONFIGURABLES-------------------------------------------------------------
	var query = "SELECT objname FROM FSCFOLIO@1.1001:Person WHERE .COOSYSTEM@1.1:usersurname IS NOT NULL";
//var query = "LIMIT 1 SELECT objname FROM FSCFOLIO@1.1001:Person";
//var query = "SELECT objname FROM COOSYSTEM@1.1:Object WHERE .COOSYSTEM@1.1:objaddress = \"COO.2130.100.6.414\"";
			...
			{//spracovanie objektov
				...
				for (var iFoundObjs = 0; iFoundObjs<foundObjs.length; iFoundObjs++)
				{
					...
					{//spracovanie osoby
						...
						foundObj.ObjectRefresh(null,true);
