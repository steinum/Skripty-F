...
try
{
	...
	//------------search--------------
	var propsQuery = "SELECT objname FROM COOSYSTEM@1.1:AttributeObjectDef WHERE .COOSYSTEM@1.1:attruseableclass IN ("+searchOT+", "+searchOT+".GetAttributeRecursive(COO.1.1.1.217)[2])";
	propsQuery += "AND (.COOSYSTEM@1.1:attrallowed.COOSYSTEM@1.1:attrallclass IN ("+searchOT+", "+searchOT+".GetAttributeRecursive(COO.1.1.1.217)[2]))"
	var propsArr = coort.SearchObjects3(cootx, propsQuery);
	
	var propsQuery = "SELECT objname FROM COOSYSTEM@1.1:AttributeObjectDef WHERE .COOSYSTEM@1.1:attruseableclass IN ("+searchOT+", "+searchOT+".GetAttributeRecursive(COO.1.1.1.217)[2])";
	propsQuery += "AND .COOSYSTEM@1.1:attrallowed.COOSYSTEM@1.1:attrallclass IS NULL"
	var propsArr1 = coort.SearchObjects3(cootx, propsQuery);
	
	if (propsArr!=null)
	{
		propsArr = propsArr.toArray();
		coort.Trace("write propsFile-propsArr-length ", propsArr.length);
	}
	
	if (propsArr1!=null)
	{
		propsArr1 = propsArr1.toArray();
		coort.Trace("write propsFile-propsArr1-length ", propsArr1.length);
		if (propsArr!=null)
		{
			for (var ipropsArr1=0; ipropsArr1<propsArr1.length; ipropsArr1++)
			{
				propsArr.push(propsArr1[ipropsArr1]);
			}
		}
		else
		{
